...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	<title>首页</title>
	<!-- 默认会参考(js,css,png)当前路径 -->
	<!-- 设置了base标签后会影响到当前页面中所有的img标签 -->
				...
				<div class="area clearfix">
					<div class="category-content" id="guide_2">
						<c:forEach items="${gtList}" var="big">
							<c:if test="${big.gparentid eq '0'}">
								<div class="category">
									<ul class="category-list" id="js_climit_li">
										<li class="appliance js_toggle relative first">
											<div class="category-info">
												<h3 class="category-name b-category-name">
													<i><img src="images/${big.gpic}"></i><a class="ml-22"
														title="点心">${big.gname}</a>
												</h3>
												<em>&gt;</em>
											</div>
														...
														<div class="menu-srot">
															<div class="sort-side">
																<c:forEach items="${gtList}" var="small">
																	<c:if test="${big.id eq small.gparentid}">
																		<dl class="dl-sort">
																			<dt>
																				<span title="${small.gname}">${small.gname}</span>
																			</dt>
																			<c:forEach items="${giList}" var="info">
																				<c:if test="${small.id eq info.goods_fatherid}">
																					<dd>
																						<a title="${info.goods_name}"
																							href="ShopServlet/getGoodsInfoById?id=${info.id}"><span>${info.goods_name}</span></a>
																					</dd>
																				</c:if>
																			</c:forEach>
																		</dl>
								...
								<c:if test="${big.id eq info.goods_parentid}">
									<div class="am-u-sm-7 am-u-md-4 text-two">
										<div class="outer-con ">
											...
											<div class="sub-title ">${info.goods_price}</div>
											<i class="am-icon-shopping-basket am-icon-md  seprate"></i>
										</div>
										<a href="GoodsInfoServlet?method=queryById&id=${info.id}"><img
